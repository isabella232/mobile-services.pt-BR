...
In the `AppDelegate` generated by Cordova, call `[ADBMobile setAdvertisingIdentifier:]` in the `application:didFinishLaunchingWithOptions:` delegate method. Para obter mais informações, consulte [Métodos de configuração](/help/ios/configuration/sdk-methods.md).
